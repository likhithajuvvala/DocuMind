...
class IngestionMetricsTest {
    ...
    void exposesTheSeriesNamesTheOverviewDashboardQueries() {
        ...
        assertThat(scrape)
                .contains(
                        "documind_ingestion_documents_total{failure=\"none\",result=\"indexed\"}");
        assertThat(scrape)
                .contains(
                        "documind_ingestion_documents_total{failure=\"permanent\",result=\"failed\"}");
        assertThat(scrape)
                .contains(
                        "documind_ingestion_documents_total{failure=\"transient\",result=\"failed\"}");
    ...
    void countsChunksAndDocumentsSeparately() {
        ...
        assertThat(
                        registry.get("documind.ingestion.documents")
                                .tag("result", "indexed")
                                .counter()
                                .count())
                .isEqualTo(2);
