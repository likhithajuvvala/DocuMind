...
/**
 * Drives the ingestion pipeline exactly as production does: publish the same {@link
 * DocumentUploadedEvent} document-service would, over a real Kafka broker, and let the app's own
 * {@code @KafkaListener} consume it. Postgres holds the metadata and the pgvector table, MinIO
 * holds the document bytes. Only the embedding provider is swapped for a deterministic local
 * stand-in so the test does not depend on a paid API.
 */
...
class IngestionPipelineIntegrationTest {
    ...
    void seedTenant() {
        ...
        workspaceRepository.save(
                new WorkspaceEntity(
                        workspaceId, "Integration Test Workspace", WorkspacePlan.TEAM, now));
        userRepository.save(
                new UserEntity(
                        userId,
                        "ingest-" + userId + "@documind.test",
                        "hash",
                        workspaceId,
                        UserRole.ADMIN,
                        now));
        ...
        // Force group assignment before the pipeline publishes anything, so
    ...
    void uploadedDocumentIsExtractedChunkedEmbeddedAndIndexed() {
        ...
        assertThat(vectorRows)
                .as("every chunk must have a matching row in the vector store")
                .isEqualTo(chunks.size());

        ConsumerRecord<String, String> indexedEvent =
                KafkaTestUtils.getSingleRecord(
                        eventConsumer, KafkaTopics.DOCUMENT_INDEXED, AWAIT_TIMEOUT);
    ...
    void reIngestingASmallerDocumentDoesNotLeaveStaleVectorsBehind() {
        ...
        String shortReplacement =
                "A single short clause about termination survives after the re-upload.";
        objectStorage.store(
        ...
        await().atMost(AWAIT_TIMEOUT)
                .untilAsserted(
                        () -> {
                            List<com.documind.common.persistence.entity.DocumentChunkEntity>
                                    chunks = chunkRepository.findByDocumentId(document.getId());
    ...
    void unparsableDocumentsAreFailedWithoutRetryingOrIndexingAnything() {
        ...
        byte[] corrupt =
                "%PDF-1.4\nthis is not a real pdf body, no xref table, no trailer."
        ...
        ConsumerRecord<String, String> failedEvent =
                KafkaTestUtils.getSingleRecord(
                        eventConsumer, KafkaTopics.DOCUMENT_FAILED, AWAIT_TIMEOUT);
    ...
    private DocumentEntity givenAnUploadedDocument(String filename, String content) {
        ...
        String storagePath =
                workspaceId + "/" + documentId + "/" + filename.replace(' ', '-') + ".txt";
        DocumentEntity document =
        ...
        if (content != null) {
            ...
            objectStorage.store(
                    storagePath, new ByteArrayInputStream(bytes), bytes.length, "text/plain");
        }
        return document;
    }
