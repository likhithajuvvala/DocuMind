...
public class UsageRecorder {
    ...
    public UsageRecorder(
            ...
            ModelPricingProperties pricing,
    ...
    /**
     * Convenience overload for callers with no provider-reported usage (e.g. the no-grounding path,
     * which never calls the model at all). Always falls back to the character estimate.
     */
    @Transactional
    ...
    public int record(AuthenticatedUser user, String prompt, String completion, Usage usage) {
        ...
        int completionTokens =
                tokenCount(usage == null ? null : usage.getCompletionTokens(), completion);
    ...
    private void publishMetrics(
            AuthenticatedUser user, int promptTokens, int completionTokens, BigDecimal cost) {
    ...
    /**
     * Prefers the count the provider actually reported; only falls back to the character heuristic
     * when a provider didn't report usage for this call (e.g. Ollama without token-count support
     * enabled, or the no-grounding path that never calls the model).
     */
    ...
    private BigDecimal estimateCost(int promptTokens, int completionTokens) {
        BigDecimal promptCost =
                pricing.getPromptCostPerThousandTokens()
        ...
        BigDecimal completionCost =
                pricing.getCompletionCostPerThousandTokens()
