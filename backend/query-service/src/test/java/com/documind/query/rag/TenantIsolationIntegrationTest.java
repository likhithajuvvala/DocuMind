 ...
 * search, not a mock. {@link ChunkRetriever} and {@link PgVectorStore} are both production classes;
 * only the embedding model is a deterministic test double, chosen so that a cross-tenant chunk can
 * be made the objectively best content match and isolation still has to hold against it.
 *
 * <p>No Spring context is started here. Booting the full query-service context would require a chat
 * model, JWT configuration, and PII redaction wiring that have nothing to do with retrieval
 * filtering, so the store and retriever are constructed directly against a Testcontainers Postgres
 * instance instead.
 */
@Testcontainers
class TenantIsolationIntegrationTest {
    ...
    static void startVectorStore() {
        ...
        vectorStore =
                PgVectorStore.builder(jdbcTemplate, new HashingEmbeddingModel(DIMENSIONS))
    ...
    private ChunkRetriever retriever(double similarityThreshold) {
        ...
        return new ChunkRetriever(
                vectorStore, properties, new SimpleMeterRegistry(), new PassThroughReranker());
    }

    private void index(
            UUID workspaceId, UUID documentId, String documentName, Integer page, String text) {
        vectorStore.add(
                List.of(
                        new Document(
    ...
    void retrievalNeverReturnsAnotherWorkspacesChunkEvenWhenItIsTheBestContentMatch() {
        ...
        index(
                ...
                "handbook.md",
                1,
                "Staff accrue twenty five days of paid annual leave per year.");
        index(
                workspaceA,
                documentA2,
                "expenses.md",
                1,
                "Expense claims must be submitted within sixty days of purchase.");
        ...
        assertThat(asWorkspaceB)
                .extracting(RetrievedChunk::documentId)
                .contains(competitorDocument);
    ...
    void documentScopedRetrievalExcludesOtherDocumentsInTheSameWorkspace() {
        ...
        index(
                ...
                "other.md",
                1,
                "The termination clause in this contract requires sixty days notice.");

        List<RetrievedChunk> scoped =
                retriever(0.0)
                        .retrieve(
                                "What does the termination clause require?",
                                workspaceId,
                                targetDocument);
    ...
    void aWorkspaceWithOnlyUnrelatedContentGetsNoMatchRatherThanAnotherWorkspacesAnswer() {
        ...
        assertThat(results)
                .as(
                        "a real miss must come back empty, never silently answered from another workspace's content")
                .isEmpty();
    }
    ...
    void aWorkspaceWithNoIndexedContentAtAllReturnsNoMatchesNotAnError() {
        ...
        List<RetrievedChunk> results =
                retriever(0.0).retrieve("Anything at all", emptyWorkspace, null);
