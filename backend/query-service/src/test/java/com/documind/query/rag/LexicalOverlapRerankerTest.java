...
class LexicalOverlapRerankerTest {
    ...
    void promotesThePassageThatAnswersTheQuestionOverTheOneThatMerelyDiscussesIt() {
        ...
        RetrievedChunk renewal = chunk(
                ...
                0.738);
        RetrievedChunk termination = chunk(
        ...
        List<RetrievedChunk> reranked =
                reranker.rerank("What is the termination clause in the vendor agreement?", List.of(renewal, termination));
    ...
    void keepsEmbeddingOrderWhenNoQuestionTermsAppearInEitherChunk() {
        ...
        List<RetrievedChunk> reranked = reranker.rerank("quarterly revenue forecast", List.of(first, second));
    ...
    void doesNotLetLexicalOverlapAloneOutweighAStrongEmbeddingMatch() {
        RetrievedChunk strongEmbedding = chunk("Annual leave accrues at twenty five days per year.", 0.95);
        RetrievedChunk keywordStuffed = chunk("leave leave leave policy leave", 0.30);

        List<RetrievedChunk> reranked = reranker.rerank("annual leave", List.of(strongEmbedding, keywordStuffed));
    ...
    void ignoresQuestionsMadeEntirelyOfCommonWords() {
        ...
        assertThat(reranker.rerank("what are the", List.of(first, second))).containsExactly(first, second);
    }
    ...
    void weightOfOneKeepsPureEmbeddingOrder() {
        ...
        List<RetrievedChunk> reranked =
                embeddingOnly.rerank("termination clause", List.of(strongEmbedding, weakButRelevantWording));
    ...
    private RetrievedChunk chunk(String text, double relevance) {
        return new RetrievedChunk(0, DOCUMENT_ID, "vendor-services-agreement.md", 1, text, relevance);
    }
}
