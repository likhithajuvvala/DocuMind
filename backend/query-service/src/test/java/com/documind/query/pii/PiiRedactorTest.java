...
class PiiRedactorTest {
    ...
    void removesContactDetailsFromTheText() {
        String text = "Escalate to alice.smith@northwind.example or call +353 1 555 0199 immediately.";

        String redacted = redactor.newSession().redact(text);

        assertThat(redacted).doesNotContain("alice.smith@northwind.example").doesNotContain("+353 1 555 0199");
    ...
    void givesTheSameValueTheSamePlaceholderWithinOnePrompt() {
        ...
        String redacted = session.redact("Contact alice@example.com. Reply to alice@example.com within a day.");
    ...
    void redactsCardNumbersThatPassLuhn() {
        String redacted = redactor.newSession().redact("Card on file 4539 1488 0343 6467 expires soon.");
    ...
    void doesNotTouchOrdinaryContractLanguage() {
        String text = "Either party may terminate with ninety (90) days notice, per clause 3.1, at 98% dispatch.";
    ...
    void redactsNationalIdentifiersAndAddresses() {
        String redacted = redactor.newSession().redact("SSN 123-45-6789 logged from 192.168.10.24 today.");
    ...
    void countsWhatItRemovedWithoutExposingTheValues() {
        ...
        assertThat(session.counts()).containsEntry(PiiCategory.EMAIL, 2).containsEntry(PiiCategory.NATIONAL_ID, 1);
    }
