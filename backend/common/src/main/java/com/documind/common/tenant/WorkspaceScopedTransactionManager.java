...
/**
 * Enables the {@code workspaceFilter} Hibernate filter (declared on the entity package, see {@code
 * package-info.java} under {@code common.persistence.entity}) on every transaction's Session as
 * soon as it begins, scoped to whatever workspace {@link WorkspaceContext} currently holds. This is
 * what turns the filter from an opt-in convenience into an enforced boundary: every query against a
 * filtered entity within the transaction — including ones that never mention a workspace, like an
 * inherited {@code findById} — is scoped, because Hibernate rewrites the generated SQL itself
 * instead of relying on each call site to remember to add one.
 *
 ...
 *
 * <p>Requests with no bound workspace (registration, login, refresh — anything before a workspace
 * is known) simply get no filter enabled, which is correct: those flows only touch unfiltered
 * entities (users, workspaces, refresh tokens) in the first place.
 *
 * <p><b>This only protects calls made inside a transaction.</b> Spring Data JPA only auto-wraps the
 * base {@code CrudRepository} methods ({@code findById}, {@code save}, {@code delete}, ...) in a
 * transaction, via {@code SimpleJpaRepository}'s own class-level {@code @Transactional} — so those
 * are covered even when called bare. A custom derived-query method declared directly on a
 * repository interface (e.g. {@code findByDocumentId}) gets no transaction of its own; it only runs
 * inside this filter's protection when the caller already wraps it in a {@code @Transactional}
