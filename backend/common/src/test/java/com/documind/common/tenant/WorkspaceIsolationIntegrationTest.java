 ...
 * workspace (that was already covered by call-site discipline) — it's the ones that don't:
 * {@code documentRepository.findById} and {@code chunkRepository.findByDocumentId} are inherited /
 ...
 */
@SpringBootTest(classes = PersistenceTestApplication.class, webEnvironment = SpringBootTest.WebEnvironment.NONE)
...
class WorkspaceIsolationIntegrationTest {
    ...
    @Autowired
    private TransactionalChunkQueries transactionalChunkQueries;
    ...
    @Test
    void findByIdIsBlockedForADocumentInAnotherWorkspaceEvenThoughTheCallSiteNeverMentionsWorkspace() {
    ...
    private DocumentEntity saveDocument(UUID workspaceId) {
        ...
        workspaceRepository.save(new WorkspaceEntity(workspaceId, "workspace-" + workspaceId, WorkspacePlan.FREE, Instant.now()));
        userRepository.save(new UserEntity(
                uploadedBy, uploadedBy + "@documind.test", "hash", workspaceId, UserRole.ADMIN, Instant.now()));
