 ...
 * database with Flyway-migrated schema. This is what actually proves rotation and reuse detection
 * work, since {@link RefreshTokenServiceTest} only proves the logic is correct against a mock, not
 * that the wiring (transactions, the unique constraint on token_hash, the exception-to-HTTP-status
 * mapping) holds together end to end.
 */
...
class AuthenticationFlowIntegrationTest {
    ...
    void rotationIssuesAFreshTokenAndTheOldOneCanNeverBeUsedAgain() {
        ...
        // a stolen token.
        ResponseEntity<ApiErrorResponse> reuseAttempt =
                restTemplate.postForEntity(
                        "/api/auth/refresh",
                        new RefreshRequest(original.refreshToken()),
        ...
        ResponseEntity<ApiErrorResponse> chainKilled =
                restTemplate.postForEntity(
                        "/api/auth/refresh",
                        new RefreshRequest(refreshed.getBody().refreshToken()),
                        ApiErrorResponse.class);
    ...
    void aNeverIssuedTokenIsRejectedAsInvalidNotAsReused() {
        ResponseEntity<ApiErrorResponse> response =
                restTemplate.postForEntity(
                        "/api/auth/refresh",
                        new RefreshRequest("this-was-never-issued-by-the-server"),
                        ApiErrorResponse.class);
    ...
    void logoutRevokesTheTokenSoItCannotBeUsedAgain() {
        ...
        ResponseEntity<Void> logoutResponse =
                restTemplate.postForEntity(
                        "/api/auth/logout", new RefreshRequest(session.refreshToken()), Void.class);
        assertThat(logoutResponse.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);

        ResponseEntity<ApiErrorResponse> afterLogout =
                restTemplate.postForEntity(
                        "/api/auth/refresh",
    ...
    void logoutIsIdempotentAndNeverLeaksWhetherATokenExisted() {
        ResponseEntity<Void> response =
                restTemplate.postForEntity(
                        "/api/auth/logout",
                        new RefreshRequest("something-that-was-never-issued"),
                        Void.class);
    ...
    void logoutAllRevokesEverySessionForTheAccountNotJustTheOnePresented() {
        ...
        assertThat(secondDeviceSession.refreshToken())
                .isNotEqualTo(firstDeviceSession.refreshToken());

        ResponseEntity<Void> logoutAll =
                restTemplate.postForEntity(
                        "/api/auth/logout-all",
                        new RefreshRequest(secondDeviceSession.refreshToken()),
                        Void.class);
        assertThat(logoutAll.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);

        ResponseEntity<ApiErrorResponse> firstDeviceAfter =
                restTemplate.postForEntity(
                        "/api/auth/refresh",
                        new RefreshRequest(firstDeviceSession.refreshToken()),
                        ApiErrorResponse.class);
        assertThat(firstDeviceAfter.getStatusCode())
                .as(
                        "logout-all must reach every session for the account, not only the family of the presented token")
                .isEqualTo(HttpStatus.UNAUTHORIZED);

        ResponseEntity<ApiErrorResponse> secondDeviceAfter =
                restTemplate.postForEntity(
                        "/api/auth/refresh",
                        new RefreshRequest(secondDeviceSession.refreshToken()),
                        ApiErrorResponse.class);
    ...
    private AuthenticationResponse register(String email) {
        RegistrationRequest request =
                new RegistrationRequest(email, "a-long-enough-password", "Workspace for " + email);
        ResponseEntity<AuthenticationResponse> response =
                restTemplate.postForEntity(
                        "/api/auth/register", request, AuthenticationResponse.class);
    ...
    private AuthenticationResponse login(String email) {
        ResponseEntity<AuthenticationResponse> response =
                restTemplate.postForEntity(
                        "/api/auth/login",
                        new LoginRequest(email, "a-long-enough-password"),
                        AuthenticationResponse.class);
